...
jira_client = get_jira_client(JIRA_API_TOKEN, JIRA_SERVER_URL, JIRA_USER_EMAIL)
...
for team, config in teams_config.items():
    if check_wip_limit(jira_client, config['board_id'], config['wip_limit'], config['states']):
        send_slack_message(slack_client, config['channel'], f"Alerta de WIP excedido para {team}")
